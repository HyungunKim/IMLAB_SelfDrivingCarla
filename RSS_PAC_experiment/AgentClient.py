...
# -- Draw Waypoints ---------------------------------------------------
# ==============================================================================
def next_waypoints(waypoint, dist = 50, ddist=1):
    waypoints = [waypoint]
    current = [waypoint]
    for i in range(int(dist/ddist)):
        new_current = []
        for w in current:
            _waypoints = w.next(ddist)
            new_current += _waypoints
            waypoints += _waypoints
        current = new_current
    return waypoints

def DrawWaypoints(waypoints, world, color=(255, 0, 0)):
    for i, w in enumerate(waypoints):
        world.debug.draw_string(w.transform.location, 'O', draw_shadow=False,
                                           color=carla.Color(r=color[0], g=color[1], b=color[2]), life_time=1.0,
                                           persistent_lines=True)
        #world.debug.draw_arrow(w, waypoints[i+1], thickness=0.5, arrow_size=0.5, life_time=10)
...
class ClientSideBoundingBoxes(object):
    ...
    @staticmethod
    def bad_driving(carla_client, in_loop=False):
        traffic_manager = carla_client.traffic_manager
        npc_list = [carla_client.world.get_actor(npc_id) for npc_id in carla_client.npc_list]
        for npc in npc_list:
            if random.uniform(0, 100) <= bad_driving_prob:
                ClientSideBoundingBoxes.bad_driver(npc, traffic_manager, in_loop=in_loop)


    @staticmethod
    def bad_driver(car, traffic_manager, in_loop=False):
        if in_loop:
            lcr = random.uniform(0, 100)
            if lcr <= lane_change_prob/2:
                traffic_manager.force_lane_change(car, True)
            elif lcr <= lane_change_prob:
                traffic_manager.force_lane_change(car, False)
        else:
            traffic_manager.ignore_lights_percentage(car, ignore_lights)
            traffic_manager.ignore_signs_percentage(car, ignore_signs)
            traffic_manager.ignore_vehicles_percentage(car, ignore_vehicles)
...
class BasicSynchronousClient(object):
    ...
    def setup_car(self):
        ...
        try:
            ...
            self.car.set_autopilot(self.use_autopilots[self.car_number])
        except:
    ...
    def draw_waypoints(self, car):
        w = self.map.get_waypoint(car.get_location())
        
        if w is not None:
            #waypoints = w.next_until_lane_end(1.5)
            waypoints = next_waypoints(w)
            DrawWaypoints(waypoints, self.world)

            wl = w.get_left_lane()
            wr = w.get_right_lane()

            if wl is not None:
                #waypointsL = wl.next_until_lane_end(1.5)
                waypointsL = next_waypoints(wl)
                DrawWaypoints(waypointsL, self.world, color=(0,255,0))
                            
            if wr is not None:
                #waypointsR = wr.next_until_lane_end(1.5)
                waypointsR = next_waypoints(wr)
                DrawWaypoints(waypointsR, self.world, color=(0,0,255))
    ...
    def game_loop(self):
        ...
        try:
            ...
            if bad_driving_prob > 0:
                ClientSideBoundingBoxes.bad_driving(self)
            ...
            while True:
                ...
                if use_waypoints:
                    self.draw_waypoints(self, self.car)
